...
public class PersonMultiResourceReader implements ItemStreamReader<Person> {

    private static final Logger LOGGER = LoggerFactory.getLogger(PersonMultiResourceReader.class);
    private SingleItemPeekableItemReader<FieldSet> singlePeekable;
    private MultiResourceItemReader<FieldSet> resourceItemReader;

    public PersonMultiResourceReader(MultiResourceItemReader<FieldSet> multiResourceItemReader) {

        this.resourceItemReader = multiResourceItemReader;
    }

    public SingleItemPeekableItemReader<FieldSet> getSinglePeekable() {
        return singlePeekable;
    }

    public void setSinglePeekable(SingleItemPeekableItemReader<FieldSet> delegate) {
        this.singlePeekable = delegate;
    ...
    public void beforeStep(StepExecution stepExecution) {
        ...
        singlePeekable = new SingleItemPeekableItemReader<>();

        String BASE_DIR = "src/main/resources/pessoas-";
        MultiResourceItemReader<FieldSet> multiReader = new MultiResourceItemReader<FieldSet>();
        Resource[] inputResources = {
                new FileSystemResource(
                        BASE_DIR + formattedString + ".S1.txt"),
                new FileSystemResource(
                        BASE_DIR + formattedString + ".S2.txt") };
        ...
        singlePeekable.setDelegate(reader);
    }

    @Override
    public void close() throws ItemStreamException {
        singlePeekable.close();
    }

    @Override
    public void open(ExecutionContext ec) throws ItemStreamException {
        singlePeekable.open(ec);
    }
    ...
    public Person read() throws Exception, UnexpectedInputException, ParseException, NonTransientResourceException {
        ...
        while ((fieldSet = singlePeekable.read()) != null) {
            ...
            FieldSet nextLine = singlePeekable.peek();
